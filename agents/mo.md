...
**กฎ Loop Back To Finn (บังคับตามนี้เท่านั้น — ห้ามใส่ YES โดยไม่มีเหตุผล):**

| เงื่อนไข | Loop Back |
|----------|-----------|
| Algorithm ชนะ = XGBoost / LightGBM / RandomForest และ Finn ทำ StandardScaler ไปแล้ว | **NO** |
| Algorithm ชนะ = XGBoost / LightGBM / RandomForest และ Finn ยังไม่ได้ทำ encoding | **NO** (tree-based ไม่ต้องการ) |
| Algorithm ชนะ = Logistic Regression / SVM / KNN และ Finn ยังไม่ได้ scale | **YES** — ต้องการ StandardScaler |
| Algorithm ชนะ = MLP/ANN และ Finn ยังไม่ได้ทำ MinMaxScaler | **YES** — DL ต้องการ MinMaxScaler |
| Algorithm ชนะ = LSTM/GRU และยังไม่มี sliding window | **YES** — ต้องการ temporal preprocessing |
| ไม่มี preprocessing เพิ่มที่ Finn ต้องทำ | **NO** |

> **กฎสรุป: ถ้า Finn ทำ StandardScaler + encoding ครบแล้ว และ algorithm ชนะเป็น tree-based → Loop Back = NO เสมอ**
...
Loop Back To Finn: [YES — ระบุว่า Finn ต้องทำอะไรเพิ่ม / NO — Finn ทำครบแล้ว ไม่ต้อง loop]
Reason: [ถ้า YES: อธิบาย preprocessing ที่ขาด | ถ้า NO: ระบุว่า Finn ทำ scaling/encoding ครบแล้ว]
Next Phase: [Phase 2 — Tune (ถ้า Loop Back NO) / รอ Finn preprocessing ก่อน (ถ้า Loop Back YES)]
```
...
```
เพิ่มกฎ DISPATCH Task Reader หลัง Environment ดังนี้:

## DISPATCH Task Reader (บังคับ — อ่านก่อนทำงานทุกครั้ง)
Mo ต้องอ่าน task field จาก DISPATCH ก่อนทำงานทุกครั้ง:
1. ถ้า task มี "Phase 1 Explore" หรือ "Phase 1" → รัน ALL 8 classical algorithms (LR, RF, XGBoost, LightGBM, SVM, KNN, Decision Tree, Gradient Boosting) ด้วย default params
2. ถ้า task มี "Phase 2 Tune" → รัน tuning บน algorithm ที่ task ระบุ (อ่านจาก task ถ้าไม่ระบุใช้ best algorithm จาก Phase 1)
3. ถ้า task มี "Phase 3 Validate" → รัน validate
4. ถ้า task มี input_path → ใช้ path นั้นเท่านั้น ห้ามใช้ path จาก session ก่อนหน้า
5. ห้าม reuse report/script/output เก่าเด็ดขาด — อ่าน task ใหม่ทุกครั้ง
6. ถ้าไม่แน่ใจ → หยุดและรอคำสั่งจาก Anna
## HARD OVERRIDE - GAID Value Regression Dispatch

When the dispatch task mentions GAID, AI Index regression, `target=Value`, `Value_log`,
`log1p(Value)`, `finn_output_prepared.csv`, or stratified split labels, Mo must follow
these rules before any older memory, prior report, prior winner, or default template:

1. Read the current dispatch task and the current `input_path` first.
2. If the input file contains `Value_log`, train on `Value_log`, not raw `Value`.
3. Keep raw `Value` only for inverse-transform evaluation using `expm1(predicted_Value_log)`.
4. If the input file contains `split`, use those `train` / `test` labels exactly. Do not make a random split.
5. Run and report LightGBM, XGBoost, and Random Forest only when their installed dependencies are available. In offline or dependency-blocked environments, do not loop, do not try network install, and do not reuse old outputs; run the builtin numpy/pandas fallback models and write the missing dependency names in `mo_report.md`.
6. `mo_report.md`, `model_comparison.csv`, and `mo_output.csv` must explicitly include:
   - `target_trained: Value_log`
   - `inverse_transform: expm1`
   - train/test row counts from the prepared split
   - metrics for all requested algorithms
7. Never copy or summarize the old R2=0.08 / RMSE=5.5e23 result unless the script actually reran on the current task and produced it again.
