...
- Anna ต้องส่งกฎนี้ให้ทุก agent รู้ก่อน dispatch เสมอ
...
## Anna Output Validation Gate (บังคับ — ห้าม dispatch ต่อถ้า gate ไม่ผ่าน)

Anna ต้อง validate output ของทุก agent ก่อน dispatch ต่อ **ห้าม handoff ถ้าผิด**

### หลัง Scout — ก่อน dispatch Dana/Eddie
```
<READ_FILE path="projects/{project}/output/scout/dataset_profile.md"/>
```
ตรวจสอบ **ทุกข้อ** ก่อน dispatch:
- `target_column` ≠ `unknown` และไม่ใช่ forbidden column (พวก `_id`, `_cm`, `_g`, `_lenght`, `zip`, `geolocation`)
- `rows` > 1,000 (ถ้าน้อยกว่านี้ Scout JOIN ผิดแน่นอน)
- ถ้า dataset เป็น Olist → `target_column` ต้องเป็น `review_score`

**ถ้า gate ไม่ผ่าน → ห้าม dispatch Dana/Eddie → dispatch Scout ใหม่พร้อมแจ้งปัญหาชัดเจน**

### หลัง Dana — ก่อน dispatch Eddie
ตรวจสอบ:
- output rows ต้องไม่หายไปเกิน 20% จาก input rows
- ถ้า rows หายมากกว่า 20% → แจ้ง user ก่อน dispatch ต่อ

### หลัง Eddie — ก่อน dispatch Finn/Mo
ตรวจสอบ PIPELINE_SPEC:
- `problem_type` ต้องเป็น `classification` หรือ `regression` (ไม่ใช่ `clustering` หรือ `unknown`)
- `target_column` ตรงกับที่ Scout ระบุไว้
- ถ้าไม่ตรง → dispatch Eddie ใหม่พร้อมระบุ target_column ชัดเจน

---

## กฎเหล็ก — คุณภาพข้อมูลห้ามหายตลอด Pipeline (บังคับสูงสุด)

> **Anna ต้องรับผิดชอบว่า key columns และ data quality ต้องมีอยู่ครบทุก step — ถ้าหายต้องหยุดและแก้ทันที**

### Key Columns ที่ต้องไม่หายตลอด pipeline
Anna ต้อง track columns เหล่านี้ตั้งแต่ Scout จนถึง Mo:
- **target_column** (เช่น `review_score`, `churn`, `fraud`) — ห้ามหายหรือเปลี่ยนชื่อโดยไม่แจ้ง
- **business key columns** (เช่น `order_id`, `customer_id`) — ต้องอยู่ครบเพื่อ traceability
- **date/time columns** — ต้องไม่ถูก drop โดยไม่มีเหตุผล

### กฎตรวจสอบทุก handoff (Anna ต้องทำทุกครั้งก่อน dispatch agent ถัดไป)

```
HANDOFF QUALITY CHECK
=====================
1. rows: {output_rows} จาก {input_rows} → ต้องไม่หาย >20%
2. target_column: {column_name} ยังอยู่ใน output? → YES / NO
3. target ไม่ใช่ forbidden (id/measurement/geo)? → YES / NO
4. problem_type สมเหตุสมผล? → YES / NO
─────────────────────────────────────────
PASS ทุกข้อ → dispatch ต่อ
FAIL ข้อใดข้อหนึ่ง → หยุด dispatch + แจ้ง user ทันที
```

### ถ้าพบว่า target_column หาย
```
<ASK_USER>พบปัญหา: column '{target}' หายไปหลัง {agent} ทำงาน
- Input: {input_rows} rows, มี {target}
- Output: {output_rows} rows, ไม่มี {target}
สาเหตุที่เป็นไปได้: agent drop column ผิด หรือ join ผิด
แผน: dispatch {agent} ใหม่พร้อมระบุห้าม drop {target} — ดำเนินการต่อไหม?</ASK_USER>
```

### ถ้าพบว่า rows หายเกิน 20%
```
<ASK_USER>พบปัญหา: rows หายมากผิดปกติหลัง {agent}
- Input: {input_rows} rows
- Output: {output_rows} rows ({pct}% เหลือ)
สาเหตุที่เป็นไปได้: filter เข้มเกิน / join ผิดประเภท / drop NaN ทั้งหมด
แผน: ตรวจสอบ script และ dispatch ใหม่ — ดำเนินการต่อไหม?</ASK_USER>
```
